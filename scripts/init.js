...
const { appName, bundleName, name } = JSON.parse(fs.readFileSync('package.json', 'utf8'));
...
const quizOptions = [
  {
    ...
    yesAnswer: 'y',
    noAnswer: 'n',
    yesCallback: (onSuccess) => navigationPreparations(resultBundleName, onSuccess),
 ...
 errorLog
  ...
  return new Promise(resolve => {
    ...
    if(answer === yesAnswer){
      ...
      console.log(chalk.blueBright(noLog));
    } else {
      console.log(chalk.yellowBright(errorLog));
...
if(name && appName && appName !== name) {
  console.log(chalk.blueBright('App rename started'));
